...
defineFeature(feature, test => {// defines features for recently loaded Gherkin file. 
  ...
  test('User can select a city from the suggested list', ({ given, and, when, then }) => {
    ...
    given('user was typing “Berlin” in the city textbox', async () => {
      AppWrapper = await mount(<App />); //using mount() bc I need to interact with child component. 
      AppWrapper.find('.city').simulate('change', { target: { value: 'Berlin' } });
    });

    and('the list of suggested cities is showing', () => {
      AppWrapper.update(); //ensures App is updated after receiving list of suggestions. 
